...
export function slackTier(slack) {
    if (slack == null || !Number.isFinite(slack))
        return 'nominal';
    if (slack < SLACK_DANGER_M)
        return 'danger';
    if (slack < SLACK_CAUTION_M)
        return 'caution';
    return 'nominal';
}
...
export function safeZonePalette(slack) {
