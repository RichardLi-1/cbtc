/**
 * Mirrors backend/topology.py — same projection, edges, switches, and train route.
 */
import type { Topology, TopologyEdge, TopologyNode } from '../types/domain'
...
function edgeLen(ax: number, ay: number, bx: number, by: number): number {
  return Math.round(Math.hypot(bx - ax, by - ay) * 100) / 100 || 1
}

export function buildYusTopology(): { topology: Topology; trainRoute: string[] } {
  const stops = (yusRaw as unknown as { line: { stops: Stop[] } }).line.stops
  ...
  const nd = (nid: string) => nodes[nid]

  const obEdges: TopologyEdge[] = []
  const ibEdges: TopologyEdge[] = []
  for (let i = 0; i < n - 1; i++) {
    const j = i + 1
    let a = nd(`ob_${i}`)
    let b = nd(`ob_${j}`)
  ...
  const xover = (eid: string, fn: string, tn: string): TopologyEdge => {
    const a = nd(fn)
    const b = nd(tn)
    return {
      id: eid,
  ...
  const xovers = [
    ...
    ...MID_XOVER_STOPS.filter(idx => idx > 0 && idx < n - 1).map(idx =>
      xover(`xover_mid_${idx}`, `ob_${idx}`, `ib_${idx}`),
    ),
  ]
