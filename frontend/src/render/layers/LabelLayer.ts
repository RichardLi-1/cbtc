import type { Topology, TopologyNode } from '../../types/domain'
...
function siblingNode(node: TopologyNode, topology: Topology): TopologyNode | null {
  if (!node.id.startsWith('ob_')) return null
...
export function renderLabelLayer(
  ...
  for (const node of topology.nodes) {
    ...
    if (!node.id.startsWith('ob_')) continue
    ...
    if (scx < -80 || scx > vp.width + 80 || scy < -80 || scy > vp.height + 80) continue
