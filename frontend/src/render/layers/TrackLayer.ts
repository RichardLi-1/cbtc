...
export function renderTrackLayer(ctx: CanvasRenderingContext2D, vp: Viewport, topology: Topology): void {
  ...
  if (vp.camera.zoom > 1) {
    for (const node of topology.nodes) {
      if (!node.is_station) continue
      const [sx, sy] = vp.worldToScreen(node.x, node.y)
      if (sx < -10 || sx > vp.width + 10) continue
      ctx.beginPath()
