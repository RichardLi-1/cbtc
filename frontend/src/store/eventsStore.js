...
useRuntimeStore.subscribe((state) => {
    ...
    for (const inc of cur.ops?.injected_events ?? []) {
        if (inc.active || inc.starts_in_s <= 0)
            continue;
        if (_seenPlannedIds.has(inc.id))
            continue;
    ...
    for (const s of cur.signals) {
        ...
        if (pv && pv !== s.aspect) {
            const sev = s.aspect === 'red' ? 'warn' : 'info';
            append({
