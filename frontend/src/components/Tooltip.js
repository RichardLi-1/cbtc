...
export function Tooltip({ hovered, topology, runtime }) {
    ...
    return (_jsxs("div", { style: {
            ...
            gap: 6,
        }, children: [isSignal && _jsx(SignalLight, { aspect: aspect }), lines.map((l, i) => {
                let color = i === 0 ? COLORS.PANEL_TEXT : COLORS.PANEL_TEXT_DIM;
                if (l.startsWith('Slack')) {
                    ...
                    if (Number.isFinite(n)) {
                        if (n < 0)
                            ...
                            color = COLORS.SIGNAL_FLASHING_YELLOW;
                        else
                            color = COLORS.SIGNAL_GREEN;
                    }
                }
                return (_jsx("div", { style: { color, fontSize: 11, fontFamily: 'monospace', lineHeight: '1.6' }, children: l }, i));
            })] }));
}
